...
def all_inputs(inp1,r1,g1,b1,r2,g2,b2,img_path,text,colors,opacity,fontsize,csv_path,cap_inp):
        
        
        if colors == "solid":
            r1, g1, b1 =int(r1), int(g1), int(b1)
        elif colors=="gradient" or colors="centere":
            r1,g1,b1 =int(r1), int(g1), int(b1)
            ...
            list_text = df["Answers"].astype(str).values.tolist()
            list_text=[s for s in text_list if s != ["Answers"]
            ...
            if cap_inp== "1":           
                font1 = ImageFont.truetype("/media/patient/02/NN/Marketing_Project/font3.ttf", head_size)

            font2 = ImageFont.truetype("/media/patient/02/NN/Marketing_Project/font3.ttf", body_size)
            counter = 1
            i = 1  
            ...
            for sents in texts:
                ...
                path = os.mkdir(path0)                       
                 ...
                 for text2 in zip(sents):
                        image = open(image_path, 'rb')
                        image_read = image.read()
                        image_64_encode = base64.encodestring(image_read)
                        image_64_decode = base64.decodestring(image_64_encode)
                        image_result = open('decoded_img.png', 'wb')
                       ...
                       if  cap_inp=="1":
                               ind=texts.index(text2)

                               text1=heading_text[ind]
...
# Formula for Making Gradient  Style      
        ...
        if colors == "solid":
            ...
            img = Image.blend(Im,Im2,opacity)
            stopwords_list = [line.rstrip('\n') for line in open("/media/patient/02/NN/Marketing_Project/stopwords")]
            ...
            font=ImageFont.truetype("/media/patient/02/NN/Marketing_Project/font3.ttf", fontsize)
            ...
            for line in lines1:
                ...
                draw.text((int((wi - width) / 2), startHeight - breather +y_text1), line, font=font, align="left", color="red")
                y_text1 += height
            g=img_path.split('.')[1]
            h=g.split('/')[-1]
            img.save("static/"+h+"blend.jpg")
            return h
        else:
            mask.putdata(mask_data)
            base.paste(top, (0, 0), mask)
...
# Take Actual Size of the Input Image & Detect Human in the Image & Make the Frame(blob) Function.
        ...
        if colors== "solid" or colors== "gradient":
#             hi, wi,c = img.shape
            aa= wi
    ...
    # Yolo Algorithm                
                ...
                colorss = np.random.uniform(0, 255, size=(len(classes), 3))
    ...
    #  Write Multiline Text with in the Box
            ...
            h=g.split('/')[-1]
            if colors=="solid":
                new_image.save("static/"+h+"_solid.jpg")
            else:
                new_image.save("static/"+h+"_gradient.jpg")
            return h
...
def test():
    ...
    if request.method == "POST":
        ...
        if colors=='solid':
            h =  all_inputs(opt1,r1,g1,b1,r2,g2,b2,app.config["IMAGE_UPLOADS"]+'/'+img.filename,text,colors,opacity,fontsize)
            
            return '<img src=' + url_for('static',filename=h+'_solid.jpg') + '>'
        elif colors=='gradient':
            h = all_inputs(opt1,r2, g2, b2, r1, g1, b1,app.config["IMAGE_UPLOADS"]+'/'+img.filename, text,colors,opacity,fontsize)
            
            return '<img src=' + url_for('static',filename=h+'_gradient.jpg') + '>'
        elif colors=='centere':
            h = all_inputs(opt1,r2, g2, b2, r1, g1, b1,app.config["IMAGE_UPLOADS"]+'/'+img.filename, text,colors,opacity,fontsize)         
            return '<img src=' + url_for('static',filename=h+'blend.jpg') + '>'
        elif colors=='ceneterd':
             return "NOT image"
